...
import (
	...
	utilassertions "go.thethings.network/lorawan-stack/v3/pkg/util/test/assertions"
		...
		t.Run(tc.Name, func(t *testing.T) {
			for _, parent := range []context.Context{
				...
				t.Run(fmt.Sprintf("%T", parent), func(t *testing.T) {
					...
					ctx, ok := utilassertions.ContextParent(ctx)
					if !tc.OK {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.Name, func(t *testing.T) {
			...
			a.So(func() {
				...
				a.So(utilassertions.ContextRoot(tc.Context), should.Equal, expected)
			}, should.NotPanic)
		})
	}
}
...
func TestContextHasParent(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.Name, func(t *testing.T) {
			a := assertions.New(t)
			a.So(func() {
				a.So(utilassertions.ContextHasParent(tc.Context, tc.Parent), should.Equal, tc.HasParent)
			}, should.NotPanic)
		})
	}
}
...
func TestShouldHaveParentContext(t *testing.T) {
	for i, tc := range []struct {
		...
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			...
			msg := utilassertions.ShouldHaveParentContext(tc.Actual, tc.Expected)
			a.So(msg, tc.Test)
			if tc.Actual == tc.Expected {
				a.So(utilassertions.ShouldHaveParentContextOrEqual(tc.Actual, tc.Expected), should.BeEmpty)
			} else {
				a.So(utilassertions.ShouldHaveParentContextOrEqual(tc.Actual, tc.Expected), tc.Test)
			}
		})
	}
}
